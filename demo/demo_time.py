...
def detect_face(img, minsize, PNet, RNet, ONet, threshold, fastresize, factor):
    ...
    # 1 #
    #####
    print("[1]:",total_boxes.shape[0], "Time:", tm.time() - t0)
    #print total_boxes
    #return total_boxes, [] 
    ...
    if numbox > 0:
        ...
        print("[2]:",total_boxes.shape[0], "Time:", tm.time() - t0)
        ...
        print("[4]:",total_boxes.shape[0], "Time:", tm.time() - t0)
        
        total_boxes[:,0:4] = np.fix(total_boxes[:,0:4])
        print("[4.5]:",total_boxes.shape[0], "Time:", tm.time() - t0)
        #print total_boxes
    ...
    if numbox > 0:
        ...
        print("[5]:",total_boxes.shape[0], "Time:", tm.time() - t0)
        #print total_boxes
        ...
        if total_boxes.shape[0] > 0:
            ...
            if len(pick) > 0 :
                total_boxes = total_boxes[pick, :]
                print("[6]:",total_boxes.shape[0], "Time:", tm.time() - t0)
                total_boxes = bbreg(total_boxes, mv[:, pick])
                print("[7]:",total_boxes.shape[0], "Time:", tm.time() - t0)
                total_boxes = rerec(total_boxes)
                print("[8]:",total_boxes.shape[0], "Time:", tm.time() - t0)
            
        #####
        # 2 #
        #####
        print("2:",total_boxes.shape, "Time:", tm.time() - t0)
        ...
        if numbox > 0:
            ...
            score = np.array([score[pass_t]]).T
            total_boxes = np.concatenate( (total_boxes[pass_t, 0:4], score), axis=1)
            print("[9]:",total_boxes.shape[0], "Time:", tm.time() - t0)
            ...
            if total_boxes.shape[0] > 0:
                ...
                print("[10]:",total_boxes.shape[0], "Time:", tm.time() - t0)
                pick = nms(total_boxes, 0.7, 'Min')
                
                #print pick
                if len(pick) > 0 :
                    total_boxes = total_boxes[pick, :]
                    print("[11]:",total_boxes.shape[0], "Time:", tm.time() - t0)
                    points = points[pick, :]

    #####
    # 3 #
    #####
    print("3:",total_boxes.shape, "Time:", tm.time() - t0)
